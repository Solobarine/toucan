...
interface SearchResult {
  id: string
  type: "user" | "post" | "hashtag" | "group" | "event"
  content: any
  relevance: number
}
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-2">
            <h3 className="font-bold text-stone-900 dark:text-stone-100 truncate">{user.name}</h3>
            {user.verified && <Verified className="w-5 h-5 text-blue-500 fill-current" />}
          </div>
          <p className="text-stone-600 dark:text-stone-400 mb-2">@{user.username}</p>
          <p className="text-stone-700 dark:text-stone-300 text-sm leading-relaxed mb-3">{user.bio}</p>
          <p className="text-stone-500 dark:text-stone-400 text-sm mb-4">{user.followers.toLocaleString()} followers</p>
          <button
      ...
      <div className="flex items-start gap-3 mb-4">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <span className="font-semibold text-stone-900 dark:text-stone-100">{post.author.name}</span>
            {post.author.verified && <Verified className="w-4 h-4 text-blue-500 fill-current" />}
            <span className="text-stone-500 dark:text-stone-400">@{post.author.username}</span>
            <span className="text-stone-500 dark:text-stone-400">·</span>
            <span className="text-stone-500 dark:text-stone-400">{post.timestamp}</span>
          </div>
        </div>
      </div>
      <p className="text-stone-900 dark:text-stone-100 leading-relaxed mb-4">{post.content}</p>
      {post.images && (
        <div className="mb-4 rounded-xl overflow-hidden">
          <img src={post.images[0] || "/placeholder.svg"} alt="" className="w-full h-48 object-cover" />
        </div>
      )}
  ...
  return (
    <div className="min-h-screen bg-stone-50 dark:bg-stone-900">
      <div className="max-w-6xl mx-auto p-6">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-stone-900 dark:text-stone-100 mb-2">Search results for "{query}"</h1>
          <p className="text-stone-600 dark:text-stone-400">Found 1,234 results in 0.12 seconds</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            {activeFilter === "all" || activeFilter === "users" ? (
              <div>
                <h2 className="text-xl font-bold text-stone-900 dark:text-stone-100 mb-4">People</h2>
            ...
            {activeFilter === "all" || activeFilter === "posts" ? (
              <div>
                <h2 className="text-xl font-bold text-stone-900 dark:text-stone-100 mb-4">Posts</h2>
                <div className="space-y-4">
                  {mockPosts.map((post) => (
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-stone-800 rounded-xl p-6 border border-stone-200 dark:border-stone-700">
              <h3 className="font-bold text-stone-900 dark:text-stone-100 mb-4">Related hashtags</h3>
              <div className="space-y-3">
                {["#webdevelopment", "#javascript", "#react", "#frontend", "#coding"].map((hashtag) => (
                  <button
                    key={hashtag}
                  ...
                  >
                    <span className="text-blue-600 dark:text-blue-400 font-medium">{hashtag}</span>
            ...
            {/* Suggested People */}
            <div className="bg-white dark:bg-stone-800 rounded-xl p-6 border border-stone-200 dark:border-stone-700">
              <h3 className="font-bold text-stone-900 dark:text-stone-100 mb-4">People you might know</h3>
              <div className="space-y-4">
                {mockUsers.slice(0, 3).map((user) => (
                  <div key={user.id} className="flex items-center gap-3">
                    ...
                    />
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-stone-900 dark:text-stone-100 truncate">{user.name}</p>
